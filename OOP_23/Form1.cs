    ...
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                float b = float.Parse(textBox2.Text);//отримання даних з textBox2

                // відрисовка функції Х
                for (float y = -pictureBox1.Height / 2; y < pictureBox1.Width / 2; y++)
                {
                    float x = -(a * (float)(Math.Sqrt(y) - Math.Sin((double)y / 50) * 50));
                    graphic.FillRectangle(new SolidBrush(Color.Yellow), x + (pictureBox1.Width / 2), -y + (pictureBox1.Height / 2), 3f, 3f);
                }

                // відрисовка функції У
                for (float x = -pictureBox1.Width / 2; x < pictureBox1.Width / 2; x++)
                {
                    float y = b * (float)(x - Math.Cos((double)x / 50) * 50);
